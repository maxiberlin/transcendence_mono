...
export class PongPaddle extends DrawObj {
     ...
     */
    constructor(paddleWidth, paddleHeight, wallDist, paddlePos, paddleSpeed, parent) {
        if (!(paddlePos === PongPaddle.PADDLE_POS_LEFT || paddlePos === PongPaddle.PADDLE_POS_RIGHT))
            throw new Error("invalid paddle Position");
        const x = paddlePos === PongPaddle.PADDLE_POS_LEFT ? wallDist : 1.0 - (paddleWidth + wallDist);
    ...
    moveUp(stop)   { console.log("stop?: ", !stop); this.dir = !stop ? -1 : this.dir === -1 ? 0 : this.dir; }
    moveDown(stop) { console.log("stop?: ", !stop); this.dir = !stop ?  1 : this.dir ===  1 ? 0 : this.dir; }
    ...
    update(elapsed) {
        ...
        if (this.dir !== 0 || elapsed === 0) {
            this.y_start = Math.floor(clamp(this.parent.y_start, nY, this.parent.y_end - this.h));
        }
    }
}
