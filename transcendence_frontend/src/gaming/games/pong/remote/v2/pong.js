...
class PongGame {
    ...
    constructor(canvas, settings) {
        ...
        this.paddleL = new GameObject(settings.paddle_width, settings.paddle_height);
        this.paddleR = new GameObject(settings.paddle_width, settings.paddle_height);
        this.scoreL = 0;
        this.scoreR = 0;
        this.running = false;
        this.prevTimestamp = 0;
        this.renderer = new GameRenderer(this.ctx, this.canvas.width, this.canvas.height, settings);
    ...
    updateGameState(buffer) {
        ...
        this.paddleL.setPosition(gameUpdate.paddle_left_x, gameUpdate.paddle_left_y);
        this.paddleL.setVelocity(gameUpdate.paddle_left_dx, gameUpdate.paddle_left_dy);
        this.paddleL.setDirection(gameUpdate.paddle_left_direction);
        this.paddleR.setPosition(gameUpdate.paddle_right_x, gameUpdate.paddle_right_y);
        this.paddleR.setVelocity(gameUpdate.paddle_right_dx, gameUpdate.paddle_right_dy);
    ...
    render(timestamp) {
        ...
        const elapsed = (this.prevTimestamp ? timestamp - this.prevTimestamp : 0) / 1000;
        this.prevTimestamp = timestamp;
        this.timeSinceLastUpdate += elapsed;

        this.renderer.clear();
        this.renderer.drawCourt(this.scoreL, this.scoreR);
        this.renderer.drawObject(this.ball, this.timeSinceLastUpdate, this.updateInterval, true);
        this.renderer.drawObject(this.paddleL, this.timeSinceLastUpdate, this.updateInterval, false);
        this.renderer.drawObject(this.paddleR, this.timeSinceLastUpdate, this.updateInterval, false);
...
class GameObject {
    ...
    interpolate(elapsed, updateInterval, isBall, settings) {
        const speed = isBall ? settings.ball_speed : settings.paddle_speed * this.direction;
...
class GameRenderer {
    ...
    drawCourt(scoreL, scoreR) {
        ...
        this.ctx.fillRect(this.borderWidth, this.borderHeight, this.width - 2 * this.borderWidth, this.height - 2 * this.borderHeight);
        ...
        this.ctx.fillText(scoreL.toString(), this.width / 2 - this.borderWidth, this.borderHeight + this.textHeight);
        this.ctx.textAlign = "left";
        this.ctx.fillText(scoreR.toString(), this.width / 2 + this.borderWidth, this.borderHeight + this.textHeight);
    }
