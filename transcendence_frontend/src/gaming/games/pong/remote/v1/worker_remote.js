...
class GameCourtRemote extends DrawObjRemote {
    ...
    constructor(settings) {
        super(
            0,
            settings.border_height,
            0,
            0,
            1.0,
            1.0 - settings.border_height * 2.0,
        );
        this.scoreL = 0;
        this.scoreR = 0;
    ...
    draw(ctx, colorWhite, colorBlack, scale_x, scale_y) {
        ...
        ctx.textAlign = 'right';
        ctx.fillText(
            this.scoreL.toString(),
            x_center - border_width_x_start,
            border_height_y_start + text_size,
        );
        ctx.textAlign = 'left';
        ctx.fillText(
...
export class PongRemote {
    ...
    initGameObjects(initMsg) {
        ...
        this.paddleL = new DrawObjRemote(
            initMsg.state.paddle_left.x,
            initMsg.state.paddle_left.y,
            initMsg.state.paddle_left.dx,
            initMsg.state.paddle_left.dy,
            this.settings.paddle_width,
            this.settings.paddle_height,
        );
        this.paddleR = new DrawObjRemote(
            initMsg.state.paddle_right.x,
            initMsg.state.paddle_right.y,
            initMsg.state.paddle_right.dx,
            initMsg.state.paddle_right.dy,
            this.settings.paddle_width,
            this.settings.paddle_height,
        );
        this.ball = new DrawObjRemote(
            initMsg.state.ball.x,
            initMsg.state.ball.y,
            initMsg.state.ball.dx,
            initMsg.state.ball.dy,
            this.settings.ball_width,
            this.settings.ball_height,
        );
    ...
    updateGameObjects(gamestate) {
        ...
        this.serverStamps.curr =
            gamestate.state.timestamp - performance.timeOrigin;

        this.serverStamps.currDelta =
            this.serverStamps.curr - this.serverStamps.last;
        this.serverStamps.deltaDiff =
            this.serverStamps.currDelta - this.serverStamps.lastDelta;

        if (
            this.settings?.tick_duration &&
            this.serverStamps.deltaDiff > 0.1 * this.settings.tick_duration
        ) {
            // console.log(
            //     'serverStamps delta diff: ',
            //     this.serverStamps.deltaDiff,
            // );
