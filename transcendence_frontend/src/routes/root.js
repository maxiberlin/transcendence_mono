...
export default class PongApp extends BaseElement {
    ...
    renderNavigation = () => {
        const ev = new ToastNotificationErrorEvent("HUHUUU")
        return html`
        <nav class="navbar ${window.innerWidth <= 576 ? 'fixed-bottom' : ''} col-12 col-sm-2 col-md-1 col-xl-1 d-flex bg-light-subtle p-0" style="height: ${window.innerWidth <= 576 ? 'auto' : '100vh'}" >
            <ul class="navbar-nav w-100 ${window.innerWidth <= 576 ? 'h-100' :''} flex-row flex-sm-column justify-content-around m-0 p-0">
                ${ PongApp.renderNavItem( 'Home', '/', 'fa-solid fa-gamepad', true ) }
                ${ PongApp.renderNavItem( 'Social', '/social/friends', 'fa-solid fa-users' ) }
                ...
                <button class="btn btn-primary" @click=${() => { this.dispatchEvent(ev); }} >
                    notify!
                </button>
            </ul>
        </nav>
    ...
    render() {
        return html`
            <div class="container-fluid bg-dark-subtle">
                <bs-toasts></bs-toasts>
                <div class="row">
                    ${!sessionService.isLoggedIn ? '' : this.renderNavigation()}
