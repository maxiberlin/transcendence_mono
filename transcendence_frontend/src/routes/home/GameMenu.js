import { BaseElem, css, html } from '../../modules.js';
...
export class BsCheck extends BaseElem {
    static observedAttributes = ["label", "name", "checked", "disabled", "type", "btn"];
    ...
    render() {
        const id = getNextNo();
        return html`
        <div class="form-check ${this.type === "switch" ? "form-switch" : ""}">
            <input @input=${{ cb:(e)=>{this.#internals.setFormValue(e.target.value);} }}
            class="form-check-input" type="${this.type === "radio" ? "radio" : "checkbox"}" role="switch"
            id="${this.name}-id-${id}" ?checked=${this.checked} ?disabled=${this.disabled} name="${this.name}">
...
export class BsRadioGroup extends BaseElem {
    ...
    connectedCallback() {
        ...
        this.shadowRoot?.querySelector("slot")?.assignedElements().forEach((elem) => {
...
export class GameMenu extends BaseElem {
    constructor() {
        super();
    }
    ...
    render() {
        return html`
            <div>
                <form action="" @submit=${this.handleForm.bind(this)} >
