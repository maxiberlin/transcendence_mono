...
export class LoginRegisterRoute extends BaseElem {
    ...
    async onBeforeMount(route, router) {
        ...
        if (sessionService.isLoggedIn) {
            if (location.pathname === '/logout') await sessionService.logout();
            return router.redirect('/');
        } else {
            if (location.pathname === '/logout') return router.redirect('/');
        }
    }
    onRouteChange(route) {
    ...
    routes = new Map([
        [
            '/login',
            { title: 'Welcome Back!', func: this.showLogin, link: '/pwforgot' },
        ],
        [
            '/register',
            {
        ...
        [
            '/pwforgot',
            { title: 'Reset your Password', func: this.showPwForgot, link: '' },
        ],
    ]);
