import { BaseElem, html, sessionService, router, renderDropdow, actions } from '../../modules.js';

export class FriendsView extends BaseElem {
    
    static get lAll()       { return "fv$all" };
    static get lReceived()  { return "fv$rec" };
    static get lSent()      { return "fv$sent" };
    static get lBlocked()      { return "fv$blocked" };
    isAll = (str) => str ? str === FriendsView.lAll : this.#lCurr === FriendsView.lAll;
    isReceived = (str) => str ? str === FriendsView.lReceived : this.#lCurr === FriendsView.lReceived;
    isBlocked = (str) => str ? str === FriendsView.lBlocked : this.#lCurr === FriendsView.lBlocked;
    isSent = (str) => str ? str === FriendsView.lSent : this.#lCurr === FriendsView.lSent;
    isRequest = (str) => this.isReceived(str) || this.isSent(str);

    #listsMap = new Map([
        [FriendsView.lAll, {title: "All Friends", prop: "friends"}],
        [FriendsView.lReceived, {title: "Incoming Requests", prop: "requestsReceived"}],
        [FriendsView.lSent, {title: "Outgoing Requests", prop: "requestsSent"}],
    ...
    constructor() {
        super(false, false)
        /** @type {Array<import('../../../types/api_data.js').UserData> | string | undefined} */
    ...
    getUserBtn = (data) => {
        if (this.isReceived())
            return html`
                ${actions.acceptFriendRequest(data.request_id, {host: this, radius: "circle", showText: false})}
        ...
        return html`
                ${sessionService.getInvited(data.id) !== undefined ? ""
                    : actions.sendGameInvitation(data.id, {host: this, radius: "circle", showText: false})}
                ${renderDropdow({color: "dark", outline: true, icon: "ellipsis-vertical"}, [
                    actions.removeFriend(data.id, {host: this, dropdownitem: true}),
                    actions.blockUser(data.id, {cb: () => {router.redirect(`/profile/${data.id}`);}, dropdownitem: true})
                ])}
            `
    }
                    ...
                    <div class=" col-5 d-block p-0 m-2">
                        <a href="/profile/${userData.id}"
                            ...
                            <avatar-component radius="3" src="${userData.avatar}" size="40">
                                <span class="m-2 text-truncate" slot="after">${userData.username}</span>
                            </avatar-component>
                        </a>
                    </div>
    ...
    getTabBtn = (listType, icon, text, activeCondition) => html`
            <button data-list="${listType}" type="button" class="btn btn-outline-dark rounded-5 text-center mx-1
    ...
    render() {
        ...
        <div class="container p-3 h-100" >
            ...
            <br>
            <div @click=${this.buttonGroupSelectHandler.bind(this)} class="btn-group w-100  z-2" role="group" aria-label="Basic outlined example" >
                ${this.getTabBtn(FriendsView.lAll, "fa-user-check", "Friends", this.isAll())}
                ${this.getTabBtn(FriendsView.lReceived, "fa-user-plus", "Requests", this.isRequest())}
                ${this.getTabBtn(FriendsView.lBlocked, "fa-ban", "Blocked", false)}
            </div>
            ...
            <div class="mt-3">${html`
                <div>
                    ${ !this.isRequest() ? "" : html`
                        <div @click=${this.buttonGroupSelectHandler.bind(this)} class="btn-group w-100  z-2" role="group" aria-label="Basic outlined example" >
                            ${this.getTabBtn(FriendsView.lReceived, "fa-envelope", "received", this.isReceived())}
                            ${this.getTabBtn(FriendsView.lSent, "fa-paper-plane", "sent", this.isSent())}
                        </div>
                    `}
                    ${ !listO ? "" : html`
                        <div >
