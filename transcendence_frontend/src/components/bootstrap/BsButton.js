...
export class BsButton extends BaseElem {
    static observedAttributes = ["color", "outline", "small", "large", "disabled", "icon", "iconright", "radius", "loadingtext", "text", "dropdownitem", "stretch"];
    constructor() {
        super(false, false);
        this.outline = false;
        this.color = "primary";
    ...
    getIcon = () => {
        ...
        return html`
            <span style="width: 1.25em;" class=" ${this.text ? "text-start" : ""} ${this.text ? `m${this.iconright ? "s" : "e"}-1` : ``} ">
                ${this._loading ? 
    ...
    render() {
        ...
        return html`
            <button @click=${this.onClick.bind(this)}
                class="${this.stretch ? "w-100" : ""} d-inline-flex align-items-center justify-content-${this.dropdownitem ? "start" : "evenly"} ${radius} btn ${color} ${size} ${this.dropdownitem ? "dropdown-item" : ""} "
                ?disabled=${this._loading} role="button">
