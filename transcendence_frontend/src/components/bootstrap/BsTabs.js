...
export class BsTabContent extends BaseElem {
    ...
    render() {
        return html`
            <div class="tab-pane" id="${this.id}-content" role="tabpanel" aria-labelledby="${this.id}-tab" tabindex="-1" style="display: ${this.active ? "block" : "none"};">
                <slot></slot>
            </div>
        `;
    }
...
export class BsTabs extends BaseElem {
    constructor() {
        super();
    }
    connectedCallback() {
    ...
    tab = (item) =>{
        ...
        const handleTabClick = (e) => {
            ...
            this.#mySlot.assignedElements().forEach((elem) => {
                if (e.target.dataset.bsTarget === `${elem.id}-content`) elem.setAttribute("active", "");
        ...
        <li class="nav-item" role="presentation">
            <button ${createTabs} @click=${handleTabClick}
                id="${item.id}-tab"
                type="button"
                role="tab"
        ...
        return html`
            <div>
                <ul class="nav nav-underline nav-justified" role="tablist">
                    ${ this.#mySlot ? this.#mySlot.assignedElements().map((item) => this.tab(item)) : "" }
                </ul>
