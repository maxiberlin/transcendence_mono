    ...
    renderStatus = (hasStatus, statusColor) => {
        ...
        return html`
            <span
                ...
                style="transform: translate(${Number(this.size) *
                0.6}px, ${Number(this.size) * -0.1}px);"
            ></span>
        `;
    };
    ...
    render() {
        ...
        return html`
            <span class="d-flex align-items-center">
                ...
                <span
                    class="position-relative d-flex flex-column"
                    style="width:${this.size ?? '0'}px; height:${this.size ??
                    '0'}px;"
                >
                    ${this.src ?
                        this.imgElem(hasStatus, statusColor)
                    :   this.svgElem(hasStatus, statusColor)}
    ...
    return html`
        ...
        >
            <avatar-component
                ...
                radius="circle"
            >
                <span class="m-2 text-truncate" slot="${side || 'after'}"
                    >${username}</span
                >
            </avatar-component>
        </a>
