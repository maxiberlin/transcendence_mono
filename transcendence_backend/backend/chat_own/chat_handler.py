...
from chat.models import ChatParticipation, ChatMessageData, ChatConversationData, ChatConversation, ChatMessage
...
async def get_conversation_data(user: UserAccount, page_number: int) -> list[ChatConversationData] | None:
    query = ChatParticipation.participants.get_user_participations(user)
    try:
        pages = Paginator(query, 10)
        page = pages.page(page_number)
        return [ q.conversation.get_conversation_data() for q in page.object_list if isinstance(q, ChatParticipation)]
    except InvalidPage:
        return None
...
# @database_sync_to_async
# def get_conversation_data(user: UserAccount) -> list[ChatConversationData]:
async def get_conversation_messages(channel_name: str, page_number: int) -> list[ChatMessageData] | None:
    try:
        conv = ChatConversation.conversations.get(channel_name=channel_name)
    except ChatConversation.DoesNotExist:
        raise ValueError("No conversation found for this ident")
    try:
        query = ChatMessage.messages.get_conversation_messages(conv)
        pages = Paginator(query, 10)
        page = pages.page(page_number)
        return [ q.get_message_data() for q in page.object_list if isinstance(q, ChatMessage)]
    except InvalidPage:
        return None

# @database_sync_to_async
# def add_message_to_conversation(p: ChatParticipation, message: str) -> ChatMessageData:
async def add_message_to_conversation(p: ChatParticipation, message: str) -> ChatMessageData:
...
class ChatHandler(BaseHandler):
    ...
    async def handle_new_message(self, command: ct.CommandPostChatMessage) -> ct.CommandResponse:
        ...
        d: ChatMessageData = await add_message_to_conversation(data[0].participant, ident)
