...
class Collision(Enum):
    ...
    @staticmethod
    def collision_detection(objA: "PongObj", objB: "PongObj") -> "Collision":
...
class PongObj:
    def __init__(self, x: float, y: float,
                 w: float, h: float,
                 dx: float, dy: float,
                 bound_top: float, bound_bottom: float,
                 bound_left: float, bound_right: float) -> None:
        self.x: float = x
        self.y: float = y
        self.w: float = w
        self.h: float = h
        self.dx: float = dx
        self.dy: float = dy
        self.bound_top: float = bound_top
        self.bound_bottom: float = bound_bottom
        self.bound_left: float = bound_left
        self.bound_right: float = bound_right

        self.speed_x: float = 1
        self.speed_y: float = 1
...
class PongBall(PongObj):
    ...
    def __init__(self, settings: PongSettings) -> None:
        ...
        self.speed_x: float = settings.ball_speed / settings.width
        self.speed_y: float = settings.ball_speed / settings.height
...
class PongPaddle(PongObj):
    class PaddlePos:
        LEFT: str = "paddle-left"
        RIGHT: str = "paddle-right"
    ...
    def __init__(self, pos: str, settings: PongSettings) -> None:
        ...
        self.speed_y: float = settings.paddle_speed / settings.height
    ...
    def update_pos_by_dir(self, tick: float):
        new_y: float = self.y + tick * self.dy * self.speed_y
        self.update_pos_direct(y=new_y)

    def set_direction(self, dir: float, release: bool):
...
#     #     # diffT = 0
#     #     # diffB = 0
#     #     diffL: float; diffR: float; diffT: float; diffB: float
