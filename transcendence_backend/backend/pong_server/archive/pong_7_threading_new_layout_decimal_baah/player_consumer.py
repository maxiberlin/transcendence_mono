...
from .game_base import PlayerConsumerBase, ClientMessage
...
class PlayerConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        try:
            ...
            self.game_schedule = await get_game_schedule(self.schedule_id)
            if self.game_schedule is None or not await is_user_part_of_game(self.game_schedule, self.user):
                await self.close()
                return
            ...
            if pong_game.players_connected < 2:
                await self.send(text_data=json.dumps(self.pong_message
                                .create_message(MessageType.WAITING_FOR_OPPONENT), cls=DecimalEncoder))
            else:
                await self.pong_message.send_to_channel_layer(
    ...
    async def forward_message(self, event):
        try:
            ...
            await self.send(text_data=json.dumps(message, cls=DecimalEncoder))
        except Exception as e:
