...
from typing import TypedDict, LiteralString, Literal, Final, get_origin
...
class PongGame(threading.Thread):
    ...
    def process_commands_sec(self, cmd: msg_client.GameEngineMessage, current_time_sec: float, max_past_timestamp_sec: float):            
            try:
                ...
                if CMD_NAME == "client-timeout":
                    ...
                    new_y = command.get("new_y", None)
                    action = command.get("action", None)
                    if isinstance(new_y, float):
                        self.player_move_queue.append(ClientMoveItem(msg=cmd, timestamp_sec=timestamp_sec, paddle=paddle, new_y=new_y, action=None))
                    elif (isinstance(action, str) and (action == "down" or action == "release_down" or action == "release_up" or action == "up")):
                        self.player_move_queue.append(ClientMoveItem(msg=cmd, timestamp_sec=timestamp_sec, paddle=paddle, new_y=None, action=action))
        ...
        if len(self.player_move_queue) > 0:
            ...
            while len(self.player_move_queue) > 0:
                ...
                if item.action:
                    ...
                    paddle.set_direction(item.action)
                elif item.new_y:
                    paddle.set_y_position(item.new_y)
    ...
    def update_regular(self, curr_time_sec: float, tick_duration_sec: float):
    ...
    def run(self) -> None:
        ...
        while self.running:
            try:
                ...
                # self.update_regular(current_time_sec, simulation_time_sec)
