    ...
    async def __game_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                print(f"elapsed: {elapsed_time_s}, curr: {(current_time_s-first_time_s)*1000}")

                if elapsed_time_s >= def_tick_duration_s:
                    tick += 1
                    print(f"update!: elapsed: {elapsed_time_s}, curr: {(current_time_s-first_time_s)*1000}, tick: {tick}")
                    ...
                    # Schlafen für den Rest der Tick-Periode
