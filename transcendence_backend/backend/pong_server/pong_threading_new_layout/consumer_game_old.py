...
class GameConsumer(AsyncConsumer):
    ...
    async def send_command_response(self, event: msg_client.GameEngineMessage, success: bool, message: str,
        ...
        await sync_to_async(self.send)(msg)
    ...
    async def join_game(self, name: str, event: msg_client.ClientJoinCommand):
        ...
        schedule_id: int = event["payload"]["schedule_id"]
        ...
        await self.games[name].player_join(user_id, schedule_id)
...
class GameHandle:
    ...
    running_user_sessions: dict[int, str] = {}
    ...
    @database_sync_to_async
    def add_user_to_game(self, user_id: int, schedule_id: int):
        ...
        if user in self.running_user_sessions:
            raise RuntimeError("User has already a running game session")
        if self.gameScheduleObj.player_one.user == user or self.gameScheduleObj.player_two.user == user:
            self.users.add(user)
            self.running_user_sessions[user.pk] = self.room_group_name
        else:
    ...
    async def player_join(self, user_id: int, schedule_id: int):
        if not self.has_game:
            await self.add_user_to_game(user_id, schedule_id)
            if len(self.users) == 2:
                self.q: queue.Queue[msg_client.ClientCommand] = queue.Queue()
                self.game = PongGame(self.settings, self.room_group_name, self.q, self.gameScheduleObj, self.channel_alias)
                print("start game!!")
                self.has_game = True
                self.game.start()
        else:
    ...
    async def quit(self):
        if self.has_game and self.game.is_alive():
            cmd: msg_client.ClientCommand = {"cmd": "client-quit-game", "id": 0, "payload": None}
            await self.push_action(cmd)
    ...
    async def push_action(self, action: msg_client.ClientCommand):
        if self.has_game and self.game.is_alive():
            ...
            print("GameHandle: push_action: Game not started")
