...
class PongGame(threading.Thread):
    def __init__(self, settings: PongSettings, channel_layer, group_name: str, q: queue.Queue[ClientCommand]):
        super().__init__()
    ...
    def determine_serve(self, score) -> ServeSide | None:
        if self.settings.serve_mode == ServeMode.WINNER:
            return ServeSide.LEFT if score == PongBall.Scored.SCORE_PLAYER_LEFT else ServeSide.RIGHT
        elif self.settings.serve_mode == ServeMode.LOSER:
            return ServeSide.RIGHT if score == PongBall.Scored.SCORE_PLAYER_LEFT else ServeSide.LEFT
        elif self.settings.serve_mode == ServeMode.RANDOM:
            return random.choice([ServeSide.LEFT, ServeSide.RIGHT])

    def reset_ball(self, serve_to: ServeSide):
        try:
    ...
    def process_player_input(self, message):
        match message["tag"]:
            case "client-move":
