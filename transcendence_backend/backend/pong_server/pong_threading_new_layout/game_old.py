...
class PongGame(threading.Thread):
    def __init__(self, settings: PongSettings, channel_layer, group_name: str, q: queue.Queue):
        super().__init__()
    ...
    def publish_game_state(self, debug=None):
        try:
            async_to_sync(self.pong_message.send_to_channel_layer)(
                ...
                {'state': self.get_game_state(debug=debug)}
            )
        except Exception as e:
            logger.error(f"Error publishing game state: {e}")
            self.handle_error(error=e)

    def handle_error(self, error_message: str | None = None, error: Exception | None = None):
        if (error):
            errstr = f"{error}"
        elif (error_message):
